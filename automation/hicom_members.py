...
class HicomStats(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.ROLE_IDS = [97711695, 97711696, 97711668, 97711706, 97711725, 82396915, 82396914]
        self.update_stats.start()

    @tasks.loop(minutes=3)
    async def update_stats(self):
        group_id = getattr(config, "group_id", None)
        channel_id = getattr(config, "hicom_members", None)
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    ...
                    new_name = f"🏆┆Hicom Members: {count}"

                    channel = self.bot.get_channel(int(channel_id))
                    if channel:
                        if channel.name != new_name:
                            await channel.edit(name=new_name)
                            print(f"[HicomStats] Channel updated: {new_name}")

        except Exception as e:
            error_msg = f"[HicomStats] Error updating stats: {e}"
            print(error_msg)
...
async def setup(bot: commands.Bot):
    await bot.add_cog(HicomStats(bot))
