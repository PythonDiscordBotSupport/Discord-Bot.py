...
class XPCog(commands.Cog):
  ...
  @xp_group.command(
      name="set",
      description="Set a specific XP amount for a member and update the Google Sheet",
  )
  @app_commands.describe(
      member="The member to set XP for",
      amount="The exact amount of XP to set (can be negative)",
      reason="Reason for setting XP",
  )
  async def xp_set(
      self,
      interaction: discord.Interaction,
    ...
    try:
      ...
      # Новое значение задается напрямую
      new_amount = amount

      # Обновляем ячейку в столбце I новым значением
      worksheet.update_cell(row, 9, new_amount)

      # 🔵 Отправка лога в канал progression (синий/нейтральный цвет для изменения)
      log_channel = self.bot.get_channel(progression)
      if log_channel:
        log_embed = discord.Embed(
            title="⚙️ XP Set",
      ...
      await interaction.followup.send(
          f"✅ Successfully set XP for {member.mention} to **{amount}**.\n"
          f"📊 Previous: `{previous_amount}` | New: `{new_amount}`",
          ephemeral=True,
      )
      ...
      error_type = type(e).__name__
      error_msg = str(e) or repr(e)
      print(f"🚨 Ошибка Google Таблиц в /xp set [{error_type}]: {error_msg}")
      
      error_channel = self.bot.get_channel(errors)
      if error_channel:
        await error_channel.send(
            f"🚨 **Google Sheets Error in XP Set:**\n"
            f"**Type:** `{error_type}`\n"
            f"**Error:** ```python\n{error_msg}\n```"
        )
